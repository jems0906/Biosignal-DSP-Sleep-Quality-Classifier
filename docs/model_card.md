...
## Inputs and method
EDF or single-column CSV signals are ingested with MNE, bandpass filtered from 0.5 to 30 Hz, notch filtered at 60 Hz when supported by the sampling rate, and transformed with Welch PSD. Relative delta, theta, alpha, and beta bandpower are the features. The deployed demo uses a serialized balanced RandomForest artifact generated by `backend/scripts/train_model.py`; the transparent rule-informed fallback remains available when the artifact is absent.
